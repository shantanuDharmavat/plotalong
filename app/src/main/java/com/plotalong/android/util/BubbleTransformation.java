...
public class BubbleTransformation implements com.squareup.picasso.Transformation {
    ...
    @Override
    public Bitmap transform(final Bitmap bmt) {
        Bitmap source = Bitmap.createScaledBitmap(bmt, 200, 200, false);
        bmt.recycle();
    ...
    public Bitmap drawMarkerBorder(Bitmap bmt) {
        Bitmap source = Bitmap.createScaledBitmap(bmt, 200, 200, false);
        bmt.recycle();
        Bitmap output = Bitmap.createBitmap(source.getWidth(), source.getHeight(), Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(output);
